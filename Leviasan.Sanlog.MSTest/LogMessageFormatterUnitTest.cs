...
namespace Leviasan.Sanlog.MSTest
{
    ...
    public sealed class LogMessageFormatterUnitTest
    {
        ...
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("DateTime: {DateTime}");
        ...
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("DateTime: {DateTime:Y}");
        ...
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("DateTime: {DateTime,-22:Y}");
        ...
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("Year month: {DateTime:Y}. StringComparison: {StringComparison:D}. Sortable date/time: {DateTime:s}.");
        ...
        {
            var format = FormattedLogValuesFormatter.MessageTemplate.Parse("0x{0:X} {0:E} {0:N}");
            ...
            format = FormattedLogValuesFormatter.MessageTemplate.Parse("{DateTime:dddd MMMM}");
        ...
        {
            var format = "{1} {0} {2}";
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse(format);
        ...
        {
            var formatException = Assert.ThrowsException<FormatException>(() => FormattedLogValuesFormatter.MessageTemplate.Parse("{1} {InvalidName} {2}"));
            Assert.AreEqual("Input string was not in a correct format. Failure to parse near offset 5.", formatException.Message);
            formatException = Assert.ThrowsException<FormatException>(() => FormattedLogValuesFormatter.MessageTemplate.Parse("{InvalidName} {0} {2}"));
            Assert.AreEqual("Input string was not in a correct format. Failure to parse near offset 15.", formatException.Message);
            formatException = Assert.ThrowsException<FormatException>(() => FormattedLogValuesFormatter.MessageTemplate.Parse("{1} {0} {InvalidName}"));
            Assert.AreEqual("Input string was not in a correct format. Failure to parse near offset 9.", formatException.Message);
            formatException = Assert.ThrowsException<FormatException>(() => FormattedLogValuesFormatter.MessageTemplate.Parse("{1} {0} {2S}"));
            Assert.AreEqual("Input string was not in a correct format. Failure to parse near offset 9.", formatException.Message);
        }
        [TestMethod]
        public void AlignmentComponent()
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("{0,-20} {1,5:N1}");
            Assert.AreEqual("Adam                  40.0", namedFormatString.Format(CultureInfo.InvariantCulture, "Adam", 40));
        }
        [TestMethod]
        public void EscapingBraces()
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("{{{0:D}}}");
            Assert.AreEqual("{6324}", namedFormatString.Format(CultureInfo.InvariantCulture, 6324));
        }
        [TestMethod]
        public void MixedNamingConvention()
        {
            var namedFormatString = FormattedLogValuesFormatter.MessageTemplate.Parse("{A1} {0A} {CC} {A1,5:X2}");
