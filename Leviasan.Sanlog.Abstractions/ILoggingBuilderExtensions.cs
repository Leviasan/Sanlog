...
namespace Leviasan.Sanlog
{
    ...
    public static class ILoggingBuilderExtensions
    {
        ...
        public static ILoggingBuilder AddSanlogLoggerFile(this ILoggingBuilder builder, Action<SanlogLoggerOptions>? loggingConfigure = default, string directory = "./", string? filePrefix = "diagnostics-", int fileSizeLimit = 10485760, int fileCountLimit = 2,
            FileFullMode strategy = FileFullMode.DropWrite, Encoding? encoding = null, bool allowSynchronousContinuations = false)
        {
            ArgumentNullException.ThrowIfNull(builder);
            // Add configuration
            builder.AddConfiguration();
            // Register logger provider
            builder.Services.TryAddSingleton(serviceProvider => new FileLoggerWriter(directory, filePrefix, fileSizeLimit, fileCountLimit, strategy, encoding, allowSynchronousContinuations));
